...
# Вариант 9: Запрограммировать обратный нечетки логический вывод на основе операции нечеткой композиции (min({1}∪{max({0}∪{xi+yi-1}|i})
...
if __name__ == '__main__':
    ...
    while cycle:
        try:
            choice = int(input("1 - Обратный логический вывод\n2 - Выход\n--->"))
        except Exception:
            print("Неверный пункт")
        else:
            if choice == 1:
                print("Формат ввода: b1: 0.5, b2: 0.6, b3: 0.2")
                consequence = input("Введите следствие: ").replace(" ", "")
                if fullmatch(r'\w{1}\d{1}:(0.\d+|0|1)(,\w{1}\d{1}:(0.\d+|0|1))*', consequence):
                    pass
                else:
                    print("Неправильный формат ввода. Вывод невозможен.")
                    continue
                print("Формат ввода: [[0.4, 0.9], [1, 0.5], [0.4, 0.9]]")
                relation = input("Введите отношение: ").replace(" ", "")
                try:
